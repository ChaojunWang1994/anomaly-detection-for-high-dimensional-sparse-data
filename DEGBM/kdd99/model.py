...
class BiWGAN(object):
    def __init__(self, input_dim, method, weight, degree, latent_dim=32, dis_fm_dim=128):
        ...
        # ==== Encoder ====
        def encoder(x_inp, is_training=False, reuse=False, getter=None):
            with tf.variable_scope('encoder', reuse=reuse, custom_getter=getter):
                with tf.variable_scope('layer_1'):
                    net = tf.layers.dense(x_inp, units=2*latent_dim, kernel_initializer=init_kernel, name='fc')
                    net = tf.nn.leaky_relu(net, alpha=leaky_relu_slope)
        ...
        # ==== Generator(Decoder) ====
        def generator(z_inp, is_training=False, reuse=False, getter=None):
        ...
        with tf.variable_scope('encoder_model'):
            z_gen = encoder(x_inp=self.input_pl, is_training=self.is_training_pl)

        with tf.variable_scope('generator_model'):
            z = tf.random_normal([batch_size, latent_dim])
            x_gen = generator(z_inp=z, is_training=self.is_training_pl)
        ...
        with tf.name_scope('loss_functions'):
            ...
            # slopes_x = tf.sqrt(tf.reduce_sum(tf.square(gradients_x), axis=1))
            # slopes_z = tf.sqrt(tf.reduce_sum(tf.square(gradients_z), axis=1))
            # gradient_penalty = tf.reduce_mean((slopes_x - 1.) ** 2) + tf.reduce_mean((slopes_z - 1.) ** 2)
        ...
        with tf.name_scope('optimizers'):
            ...
            optimizer_gen = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.5, beta2=0.9, name='gen_optimizer')
            optimizer_enc = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.5, beta2=0.9, name='enc_optimizer')

            with tf.control_dependencies(update_ops_dis):
                dis_op = optimizer_dis.minimize(self.loss_discriminator, var_list=dvars)
            with tf.control_dependencies(update_ops_gen):
                gen_op = optimizer_gen.minimize(self.loss_generator, var_list=gvars)
            with tf.control_dependencies(update_ops_enc):
                enc_op = optimizer_enc.minimize(self.loss_encoder, var_list=evars)
        ...
        # ================== Build testing graph ==================
        with tf.variable_scope('encoder_model'):
            z_gen_ema = encoder(x_inp=self.input_pl, is_training=self.is_training_pl,
                                getter=get_getter(enc_ema), reuse=True)

        with tf.variable_scope('generator_model'):
            x_gen_ema = generator(z_inp=z_gen_ema, is_training=self.is_training_pl,
                                  getter=get_getter(gen_ema), reuse=True)

        with tf.variable_scope('discriminator_model'):
            logits_dis_enc_ema, fm_layer_inp_ema = discriminator(z_inp=z_gen_ema,
                                                                 x_inp=self.input_pl,
                                                                 is_training=self.is_training_pl,
                                                                 getter=get_getter(dis_ema),
                                                                 reuse=True)
            logits_dis_gen_ema, fm_layer_rct_ema = discriminator(z_inp=z_gen_ema, x_inp=x_gen_ema,
                                                                 is_training=self.is_training_pl,
                                                                 getter=get_getter(dis_ema),
                                                                 reuse=True)
