...
# from New_model import BiWGAN
# from model import BiWGAN
from skip_GANomaly import BiWGAN
...
with tf.Session() as sess:
    ...
    percentiles = [i/10 for i in range(785, 821)]
    ...
    for p in percentiles:
        ...
        prec, rec, f1 = calc_metric(y_test, ano_scores, percentile=p)

        # Calculate auc
        auprc = calc_auc(y_test, ano_scores)
        print('Prec:{:.4f}  |  Rec:{:.4f}  |  F1:{:.4f}    ({})'.format(prec, rec, f1, p))
