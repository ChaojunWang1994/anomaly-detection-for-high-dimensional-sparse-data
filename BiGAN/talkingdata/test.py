...
def evaluation(sess, model):
    ...
    draw_prc(y, ano_scores, key='EfficientGAN_'+'cross-e')


with tf.Session() as sess:
    model = BiGAN(count_list, method, weight=weight, degree=degree)
    sess.run(tf.global_variables_initializer())
    sess.run(tf.local_variables_initializer())
    model.restore(sess, 'train_logs_0.8427_fm/talkingdata/fm/0.9/1/ckpt')
