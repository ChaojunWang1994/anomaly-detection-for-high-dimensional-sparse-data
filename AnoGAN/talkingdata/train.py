...
nb_epochs = 100
print_iter = 500
d_g_iter = 2  # (D's training epochs / G's training epochs = d_g_iter - 1)
learning_rate = 1e-4
method = 'cross-e'  # or 'cross-e'
weight = 0.9
degree = 1
...
with tf.Session() as sess:
    model = AnoGAN(count_list, method, weight=weight, degree=degree)
    ...
    for i in range(nb_epochs):
        ...
        x, y = _shuffle(x, y)
        loss_dis_sum, loss_gen_sum = 0., 0.
        for j, batch_data in DataInput(x, train_batch_size):
            # train D
            if (j % d_g_iter != 0) or (j == 0):
                loss_dis, loss_gen = model.train(sess, batch_data, learning_rate, train_d=True)
            # train G, E
            else:
                loss_dis, loss_gen = model.train(sess, batch_data, learning_rate, train_d=False)
            loss_dis_sum += loss_dis
            loss_gen_sum += loss_gen

            print_iter = 500 if best_auroc > 0.78 else 1000
            if j % print_iter == 0:
                print('== Epoch {}  Batch {}\tLoss_dis:{:.4f}  Loss_gen:{:.4f} =='\
                      .format(i, j,
